...
@registry.BACKBONES.register("PVA-FPN-RETINANET")
def build_inception_fpn_p3p7_backbone(cfg):
    body = inception.PVANET_LITE(cfg)
    fpn_channel_shrink = cfg.MODEL.RESNETS.BACKBONE_OUT_CHANNELS_SHRINK
    in_channels_stage2 = cfg.MODEL.RESNETS.RES2_OUT_CHANNELS
    out_channels = cfg.MODEL.RESNETS.BACKBONE_OUT_CHANNELS
    in_channels_p6p7 = in_channels_stage2 * 8 if cfg.MODEL.RETINANET.USE_C5 \
        else out_channels
    fpn = fpn_module.FPN(
        ...
        out_channels=out_channels//fpn_channel_shrink,
        conv_block=conv_with_kaiming_uniform(
            cfg.MODEL.FPN.USE_GN, cfg.MODEL.FPN.USE_RELU
        ),
        top_blocks=fpn_module.LastLevelP6P7(in_channels_p6p7, out_channels//fpn_channel_shrink),
    )
    model = nn.Sequential(OrderedDict([("body", body), ("fpn", fpn)]))
    model.out_channels = out_channels//fpn_channel_shrink
    return model
